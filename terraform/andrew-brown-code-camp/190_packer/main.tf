terraform {
  required_providers {
    aws = {
      source  = "hashicorp/aws"
      version = "4.45.0"
    }
  }
}

provider "aws" {
  region = "us-east-1"
}

data "aws_ami" "packer_image" {
    executable_users = ["self"]
    # most_recent = true
    # name_regex = "^my-server-.*" #this won't actually work; the packer image from packer.pkr.hcl didn't create
    filter { 
        name = "name"
        values = ["my-server-httpd"] # example name from ABCC, not what I put in packer 
    }
    owners = ["self"]
}

variable "instance_type" {
    type = string
    default = "t2.micro"
}

resource "aws_key_pair" "deployer" {
  key_name   = "deployer_key"
  public_key = "ssh-rsa "
}


resource "aws_instance" "my_server" {
  count = 2
  ami                    = data.aws_ami.packer.id
  instance_type          = var.instance_type
  key_name               = aws_key_pair.deployer.key_name

  tags = {
    Name = "MyServer-${count.index}"
  }
}


data "aws_vpc" "main" {
  id = "vpc-b44a2dce"
} 

output "public_ip" {
  value = aws_instance.my_server[*].public_ip
}